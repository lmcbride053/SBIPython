...
# Program Structure
<pre> .
├── data/           # Input folder containing PDB files
├── examples/       # Contains example runs
│   ├── results/    # Output data for example PDB files
│   └── pdbs/       # PDB files used in the examples
├── results/        # Output PyMOL ".pml" scripts generated by the pipeline
├── scripts/        # All core scripts used in the pipeline (executed via "main.py")
└── main.py         # Main driver script that executes the pipeline steps
</pre>
